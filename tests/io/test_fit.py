...
def test_top_level_import():
    assert chironpy.read_fit == fit.read_fit


@pytest.mark.parametrize(
    "example",
    [(i) for i in chironpy.examples(file_type=FileTypeEnum.fit, course=False)],
)
...
def test_read_fit_no_fit():
    example_tcx = chironpy.examples(path="activity_4078723797.tcx")
    with pytest.raises(exceptions.InvalidFitFile):
        chironpy.read_fit(example_tcx.path)


@pytest.mark.parametrize(
    "example", [(i) for i in chironpy.examples(file_type=FileTypeEnum.fit, course=True)]
)
def test_read_fit_course(example):
    course = chironpy.read_fit(example.path)
