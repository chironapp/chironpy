...
from chironpy.io import gpx
from chironpy.examples.utils import FileTypeEnum


def test_top_level_import():
    assert chironpy.read_gpx == gpx.read_gpx


@pytest.mark.parametrize(
    "example",
    [(i) for i in chironpy.examples(file_type=FileTypeEnum.gpx, course=False)],
)
...
@pytest.mark.parametrize(
    "example", [(i) for i in chironpy.examples(file_type=FileTypeEnum.gpx, course=True)]
)
